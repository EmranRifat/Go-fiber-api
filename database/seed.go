...
func SeedListingFromJSON(db *gorm.DB) error {
	...
	for _, l := range listingsData {
		listingID := uuid.NewSHA1(uuid.NameSpaceURL, []byte("listing:"+l.ID))
		...
		images := l.Images
		if len(images) == 0 && strings.TrimSpace(l.Image) != "" {
			images = []string{l.Image}
		}

		image := l.Image
		if strings.TrimSpace(image) == "" && len(images) > 0 {
			image = images[0]
		}

		host := l.Host
		if strings.TrimSpace(host.Name) == "" {
			host = models.Host{Name: l.HostName, IsSuperhost: l.IsSuperhost}
		}
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		insertedRows = createResult.RowsAffected

		return nil
	})
	if err != nil {
		return fmt.Errorf("failed to replace seeded listings: %w", err)
	}
